...
jest.mock("../../services/unsplash", () => {
  const sampleResult = [
    { id: 1, description: "This is pic 1", urls: { small: "http://image" } },
...
test("ensure fetch called upon load", async () => {
  ...
  await wait(() => {
    expect(getByText("This is pic 1")).toBeInTheDocument();
    expect(getByText("This is pic 2")).toBeInTheDocument();
    expect(getByText("This is pic 3")).toBeInTheDocument();
  });
}); 